...
## Описание
Эта система использует компьютерное зрение и машинное обучение для автоматического обнаружения лиц в видеопотоке и классификации правильности ношения медицинских масок. Система поддерживает три класса классификации: маска надета правильно, маска надета неправильно и маска отсутствует.
...
## Архитектура системы
### Компоненты
webApp.py: Основное FastAPI приложение с управлением жизненным циклом

api.py: REST API эндпоинты для взаимодействия с системой

camera.py: Менеджер захвата видео и управления камерами

enhanced_mask_detector.py: Детектор лиц и классификатор масок

augment_train_improved_masks_pytorch.py: Система обучения модели классификации
...
pip install fastapi uvicorn jinja2 opencv-python mediapipe torch torchvision numpy
...
Быстрый старт с камерой 0
bash
python webApp.py --camera 0 --width 640 --height 480
С улучшенным сглаживанием
bash
python webApp.py --alpha 0.85 --max_age 20 --min_iou 0.3
Обучение модели на пользовательских данных
bash
python augment_train_improved_masks_pytorch.py
Технические детали
